...
public class BmiCalculator {
	 ...
	 * @param imperial
	 *            If true, use pounds/inches instead
	 * @return BMI value, or -1 if there was an error
	 */
	public double calculateBmi(double weight, double height, boolean imperial) {

		// The BMI calculator service uses metric units, so imperial units must
		// be converted to metric
		if (imperial) {
			ConverterServiceSoapProxy converterProxy = new ConverterServiceSoapProxy(
					"http://www.beetledev.com/webservices/converterservice.asmx");
		...
		// Now that the weight and height are in the correct units, use the BMI
		// calculator service
